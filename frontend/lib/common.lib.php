...
function insert_adcp($uid, $mb_id, $admob_type, $ru_ad_ea_v, $etc) //룰렛 애드몹 CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    if (!$mb['mb_id']) { return 0; }		
		...
		if ($admob_type == "lobby_key_ad") { // 로비 광고 시청시 카운트 계산
			$sql_admob_count = sql_fetch(" SELECT admob_count FROM 0_edu_coquiz_g5_adcp WHERE mb_id = '$mb_id' and admob_type = 'lobby_key_ad' ORDER BY admob_id DESC LIMIT 1");
			if($sql_admob_count["admob_count"] == 3) { $admob_count_v = 1;} else {$admob_count_v = $sql_admob_count["admob_count"] + 1;}
		}
		
		$sql = " insert into 0_edu_coquiz_g5_adcp
					set uid = '$uid',
						mb_id = '$mb_id',						
						admob_type = '$admob_type',
						admob_count = '$admob_count_v',
						etc = '$etc',
						admob_datetime = '".G5_TIME_YMDHIS."'
						 ";
		//echo $sql; die();
		$sql_query = sql_query($sql);
		
		$sql_admob_root = sql_fetch(" SELECT admob_id FROM 0_edu_coquiz_g5_adcp WHERE mb_id = '$mb_id' ORDER BY admob_id DESC LIMIT 1"); //시청한 admob id 구하기
		
		$ins_key_admob = insert_roulette($sql_admob_root["admob_id"], $mb_id, $ru_ad_ea_v, "쿠팡 AD 보상", $mb_id, "admob", $admob_type);	
...
// 아이템
//하루 한 줄 돈줄 퀴즈 아이템 - insert_item(0, $member["mb_id"], $step_quiz_item_info, -1, '하루하루 돈쭐 퀴즈', $member["mb_id"], 'quiz', $quiz_cate);	
function insert_item($uid=0, $mb_id, $item_type, $item_ea, $item_content, $item_action_id, $item_action_type, $item_action_type_detail) //CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    $sql = " insert into 0_edu_coquiz_g5_item
                set uid = '$uid',
					mb_id = '$mb_id',
					item_type = '$item_type',
					item_ea = '$item_ea',
					...
					item_action_type_detail = '$item_action_type_detail',
                    item_datetime = '".G5_TIME_YMDHIS."'
                     ";
	//echo $sql; die();
    sql_query($sql);
    ...
    // 회원 아이템 UPDATE
	$mb_item_colum = "mb_".$item_type;	
	if ($item_ea > 0) {$item_ea = "+".$item_ea;}
    $sql = " update {$g5['member_table']} set $mb_item_colum = $mb_item_colum {$item_ea} where mb_id = '{$mb_id}' ";
	sql_query($sql);
...
// 쿠키
//박스 오픈 - insert_cookie($_GET["box_id"], $member["mb_id"], $mb_cookie, "코넛박스 OPEN", $member["mb_id"], "conutbox", "lobby_conutbox_open");
function insert_parts($uid, $mb_id, $parts_use_profil, $parts_section, $parts_detail, $parts_get_type, $parts_get_type_detail, $parts_use_nft) //CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    if (!$mb['mb_id']) { return 0; }
		...
		$sql = " insert into 0_edu_coquiz_g5_parts
					set uid = '$uid',
						mb_id = '$mb_id',
						parts_use_profil = '$parts_use_profil',
						parts_section = '$parts_section',
						parts_detail = '$parts_detail',
						parts_get_type = '$parts_get_type',
						parts_get_type_detail = '$parts_get_type_detail',
						parts_use_nft = '$parts_use_nft',
						parts_datetime = '".G5_TIME_YMDHIS."'
						 ";
		sql_query($sql);
...
// 박스
//하루 한 줄 돈줄 퀴즈 아이템 - insert_item($member["mb_id"], $step_quiz_item_info, -1, '하루하루 돈쭐 퀴즈', $member["mb_id"], 'quiz');
function insert_box($uid, $mb_id, $box_type, $box_ea, $box_content, $box_action_id, $box_action_type, $box_action_type_detail, $etc="") //CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    // 포인트가 없다면 업데이트 할 필요 없음
    if ($box_ea == 0) { return 0; }

    // 회원아이디가 없다면 업데이트 할 필요 없음
    if ($mb_id == '') { return 0; }
    $mb = sql_fetch(" select mb_id,box from {$g5['member_table']} where mb_id = '$mb_id' ");
    if (!$mb['mb_id']) { return 0; }
	
				if ($box_action_type == "quiz") {
					
					// 이미 등록된 내역이라면 건너뜀
					$sql = " select count(*) as cnt from 0_edu_coquiz_g5_box where mb_id = '$mb_id' and box_content = '$box_content'";
					//echo $sql;
					$row = sql_fetch($sql);
					//echo "이미 등록2";
					if ($row['cnt']) {
	...
	if ($mb['box'] < 5) { 
		...
		$sql = " insert into 0_edu_coquiz_g5_box
					set uid = '$uid',
						mb_id = '$mb_id',
						box_type = '$box_type',
						box_ea = '$box_ea',
						box_content = '".addslashes($box_content)."',
						box_action_id = '$box_action_id',
						box_action_type = '$box_action_type',
						box_action_type_detail = '$box_action_type_detail',
						box_datetime = '".G5_TIME_YMDHIS."',
						box_open = '0',
						etc = '$etc'
						 ";
		//echo $sql;
		...
		//회원 박스 수량 업데이트
		$sql = "update {$g5['member_table']} SET box = box + 1 where mb_id = '$mb_id'";
		sql_query($sql);
...
/*
    // 아이템 UPDATE
	$mb_item_colum = $mb_item_colum = "mb_".$item_type;	
    $sql = " update {$g5['member_table']} set $mb_item_colum = $mb_item_colum {$item_ea} where mb_id = '{$mb_id}' ";
	sql_query($sql);
*/
    return 1;
...
// 쿠키
//박스 오픈 - insert_cookie($_GET["box_id"], $member["mb_id"], $mb_cookie, "코넛박스 OPEN", $member["mb_id"], "conutbox", "lobby_conutbox_open");
function insert_cookie($uid, $mb_id, $cookie_ea, $cookie_content, $cookie_action_id, $cookie_action_type, $cookie_action_type_detail) //CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    if (!$mb['mb_id']) { return 0; }
		...
		$sql = " insert into 0_edu_coquiz_g5_cookie
					set uid = '$uid',
						mb_id = '$mb_id',
						cookie_ea = '$cookie_ea',
						cookie_content = '".addslashes($cookie_content)."',
						cookie_action_id = '$cookie_action_id',
						cookie_action_type = '$cookie_action_type',
						cookie_action_type_detail = '$cookie_action_type_detail',
						cookie_datetime = '".G5_TIME_YMDHIS."'
						 ";
		sql_query($sql);
		
		if ($cookie_ea > 0 ) {$ea_plus = "+";}
		
		$sql = "update {$g5['member_table']} SET mb_cookie = mb_cookie ".$ea_plus.$cookie_ea." where mb_id = '$mb_id'"; //회원 쿠키 수정
		//echo $sql;
		sql_query($sql);
...
// 사탕
//박스 오픈 - insert_candy($_GET["box_id"], $member["mb_id"], $mb_candy, "코넛박스 OPEN", $member["mb_id"], "conutbox", "lobby_conutbox_open");
//일일 퀘스트1 - $ins_candy_box = insert_candy(0, $member["mb_id"], 20, "일일 퀘스트 완료", $member["mb_id"], "quest", "quest_day_1");
function insert_candy($uid, $mb_id, $candy_ea, $candy_content, $candy_action_id, $candy_action_type, $candy_action_type_detail) //CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    if (!$mb['mb_id']) { return 0; }
		...
		$sql = " insert into 0_edu_coquiz_g5_candy
					set uid = '$uid',
						mb_id = '$mb_id',
						candy_ea = '$candy_ea',
						candy_content = '".addslashes($candy_content)."',
						candy_action_id = '$candy_action_id',
						candy_action_type = '$candy_action_type',
						candy_action_type_detail = '$candy_action_type_detail',
						candy_datetime = '".G5_TIME_YMDHIS."'
						 ";
		sql_query($sql);
		
		if ($candy_ea > 0 ) {$ea_plus = "+";}
		
		$sql = "update {$g5['member_table']} SET mb_candy = mb_candy + ".$ea_plus.$candy_ea." where mb_id = '$mb_id'"; //회원 캔디 수정
		sql_query($sql);
...
// 파이
//insert_pie('', "test_coquiz", -1, "파이 테스트", "test_coquiz", "test", "test");
function insert_pie($uid, $mb_id, $pie_ea, $pie_content, $pie_action_id, $pie_action_type, $pie_action_type_detail, $etc='') //CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    if (!$mb['mb_id']) { return 0; }
		...
		$sql = " insert into 0_edu_coquiz_g5_pie
					set uid = '$uid',
						mb_id = '$mb_id',
						pie_ea = '$pie_ea',
						pie_content = '".addslashes($pie_content)."',
						pie_action_id = '$pie_action_id',
						pie_action_type = '$pie_action_type',
						pie_action_type_detail = '$pie_action_type_detail',
						etc = '$etc',
						pie_datetime = '".G5_TIME_YMDHIS."'
						 ";
		//echo $sql;
		sql_query($sql);
		...
		$sql = "update {$g5['member_table']} SET mb_pie = mb_pie + ".$ea_plus.$pie_ea." where mb_id = '$mb_id'"; //회원 캔디 수정
		sql_query($sql);
...
// 룰렛
//insert_pie('', "test_coquiz", -1, "파이 테스트", "test_coquiz", "test", "test");
function insert_roulette($uid, $mb_id, $roulette_ea, $roulette_content, $roulette_action_id, $roulette_action_type, $roulette_action_type_detail) //CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    if (!$mb['mb_id']) { return 0; }
		...
		$sql = " insert into 0_edu_coquiz_g5_roulette
					set uid = '$uid',
						mb_id = '$mb_id',
						roulette_ea = '$roulette_ea',
						roulette_content = '".addslashes($roulette_content)."',
						roulette_action_id = '$roulette_action_id',
						roulette_action_type = '$roulette_action_type',
						roulette_action_type_detail = '$roulette_action_type_detail',
						roulette_datetime = '".G5_TIME_YMDHIS."'
						 ";
		//echo $sql;
		sql_query($sql);
		
		if ($roulette_ea > 0 ) {$ea_plus = "+";}
		
		$sql = "update {$g5['member_table']} SET mb_roulette = mb_roulette + ".$ea_plus.$roulette_ea." where mb_id = '$mb_id'"; //회원 캔디 수정
		sql_query($sql);
...
// 키
//키 오픈 - insert_key($_GET["box_id"], $member["mb_id"], $mb_key, "코넛박스 OPEN", $member["mb_id"], "conutbox", "lobby_conutbox_open");
function insert_key($uid, $mb_id, $key_ea, $key_content, $key_action_id, $key_action_type, $key_action_type_detail) //CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    if (!$mb['mb_id']) { return 0; }
		...
		$sql = " insert into 0_edu_coquiz_g5_key
					set uid = '$uid',
						mb_id = '$mb_id',
						key_ea = '$key_ea',
						key_content = '".addslashes($key_content)."',
						key_action_id = '$key_action_id',
						key_action_type = '$key_action_type',
						key_action_type_detail = '$key_action_type_detail',
						key_datetime = '".G5_TIME_YMDHIS."'
						 ";
		sql_query($sql);
		
		$sql = "update {$g5['member_table']} SET mb_key = mb_key + ".$key_ea." where mb_id = '$mb_id'"; //회원 키 수정
		sql_query($sql);
...
//
function insert_admob($uid, $mb_id, $admob_type, $etc) //애드몹 CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    if (!$mb['mb_id']) { return 0; }		
		...
		if ($admob_type == "lobby_key_ad") { // 로비 광고 시청시 카운트 계산
			$sql_admob_count = sql_fetch(" SELECT admob_count FROM 0_edu_coquiz_g5_admob WHERE mb_id = '$mb_id' and admob_type = 'lobby_key_ad' ORDER BY admob_id DESC LIMIT 1");
			if($sql_admob_count["admob_count"] == 5) { $admob_count_v = 1;} else {$admob_count_v = $sql_admob_count["admob_count"] + 1;}
		}
		
		$sql = " insert into 0_edu_coquiz_g5_admob
					set uid = '$uid',
						mb_id = '$mb_id',						
						admob_type = '$admob_type',
						admob_count = '$admob_count_v',
		...
		$sql_admob_root = sql_fetch(" SELECT admob_id FROM 0_edu_coquiz_g5_admob WHERE mb_id = '$mb_id' ORDER BY admob_id DESC LIMIT 1"); //시청한 admob id 구하기
		
		if ($admob_type == "lobby_key_ad") {	
			//admob 광고시청 열쇠 지급
			$ins_key_admob = insert_key($sql_admob_root["admob_id"], $mb_id, 1, "로비 AD 보상", $mb_id, "admob", $admob_type);			
		} else if ($admob_type == "market_key_ad") {	
			//admob 광고시청 열쇠 지급
			$ins_key_admob = insert_key($sql_admob_root["admob_id"], $mb_id, 2, "상점 AD 보상", $mb_id, "admob", $admob_type);
		} else if ($admob_type == "market_item_ad") {
			//admob 광고시청 아이템 지급
			$ins_pass_admob = insert_item($sql_admob_root["admob_id"], $mb_id, "pass", 1, "상점 AD 보상", $mb_id, "admob", $admob_type);
			$ins_time_admob = insert_item($sql_admob_root["admob_id"], $mb_id, "time", 1, "상점 AD 보상", $mb_id, "admob", $admob_type);
			$ins_x2_admob = insert_item($sql_admob_root["admob_id"], $mb_id, "x2", 1, '상점 AD 보상', $mb_id, "admob", $admob_type);
		} else if ($admob_type == "market_cookie_ad") {
			//admob 광고시청 아이템 지급
			$ins_cookie_admob = insert_cookie($sql_admob_root["admob_id"], $mb_id, 10, "상점 AD 보상", $mb_id, "admob", $admob_type);											
		}
...
function insert_quest($uid, $mb_id, $quest_content, $quest_type, $quest_type_detail, $etc) //quest CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    if (!$mb['mb_id']) { return 0; }
		...
		$sql = " insert into 0_edu_coquiz_g5_quest
					set uid = '$uid',
						mb_id = '$mb_id',
						quest_content = '$quest_content',					
						quest_type = '$quest_type',
						quest_type_detail = '$quest_type_detail',
						etc = '$etc',
						quest_datetime = '".G5_TIME_YMDHIS."'
						 ";
		$sql_query = sql_query($sql);
		
		/*
		if ($admob_type == "lobby_key_ad") {		
			$sql_admob_root = sql_fetch(" SELECT admob_id FROM 0_edu_coquiz_g5_admob WHERE mb_id = '$mb_id' ORDER BY admob_id DESC LIMIT 1");		
			//admob 광고시청 열쇠 지급
			$ins_key_admob = insert_key($sql_admob_root["admob_id"], $mb_id, 2, "로비 광고 시청 보상", $mb_id, "admob", $admob_type);
		}
		*/
...
{
    global $sst, $sod, $sfl, $stx, $sfl2, $stx2, $page, $sca;
...
// 포인트 부여
function insert_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0, $quiz_correct=0, $quiz_time=0) //CONUT
{
    global $config;
    global $g5;
    global $is_admin;
    ...
    if ($rel_action == "quiz" or  $rel_action == "prediction" or  $rel_table == "@quest" or  $rel_table == "@roullet") {//CONUT
            /*재등록 방지 패스
            $sql = " select count(*) as cnt from {$g5['point_table']}
                      where po_datetime >= NOW() - INTERVAL 30 SECOND and mb_id = '$mb_id' and po_point = '$point' and po_rel_table = '$rel_table' and po_rel_action = '$rel_action'";//30초안에 재등록? 해킹?
                        //echo $sql; exit;
            $row = sql_fetch($sql);
            //echo $sql;
            if ($row['cnt']) {
            ...
            */
			...
			if ($rel_action == "quiz") {
				// 이미 등록된 내역이라면 건너뜀
				$sql = " select count(*) as cnt from {$g5['point_table']} where mb_id = '$mb_id' and po_content = '$content'";
				//echo $sql;
				$row = sql_fetch($sql);
				//echo "이미 등록2";
				if ($row['cnt']) {
    ...
    if ($rel_action == "quiz" or  $rel_action == "prediction") {//CONUT
    	$top_po_id = sql_fetch("select po_id from {$g5['point_table']} where mb_id = '$mb_id' order by po_id DESC LIMIT 1");        
    	return $top_po_id["po_id"];
    } else {
    	return 1;
    }
}
...
function get_sideview($mb_id, $name='', $email='', $homepage='')
{
	...
	*/
    ...
    if ($mb_id) {
        //$menus['profile'] = '<a href="member_list.php?sfl=mb_id&stx=kakao_8cdd08ed" onclick="win_profile(this.href); return false;" rel="nofollow">자기소개</a>';
		$menus['profile'] = '<a href="member_list.php?sfl=mb_id&stx='. $mb_id .'" rel="nofollow" target="_blank">회원정보</a>';
    }
    ...
    if ($mb_id) {
        ...
        if ($is_admin == 'super') {
           // $menus['admin_member_modify'] = '<a href="' . G5_ADMIN_URL . '/member_form.php?w=u&amp;mb_id=' . $mb_id . '" target="_blank" rel="nofollow">회원정보변경</a>';
            $menus['admin_member_point'] .= '<a href="' . G5_ADMIN_URL . '/point_list.php?sfl=mb_id&amp;stx=' . $mb_id . '" target="_blank" rel="nofollow">포인트</a>';
			$menus['admin_member_point'] .= '<a href="' . G5_ADMIN_URL . '/pie_list.php?sfl=mb_id&amp;stx=' . $mb_id . '" target="_blank" rel="nofollow">파이</a>';
			$menus['admin_member_point'] .= '<a href="' . G5_ADMIN_URL . '/key_list.php?sfl=mb_id&amp;stx=' . $mb_id . '" target="_blank" rel="nofollow">열쇠</a>';
			$menus['admin_member_point'] .= '<a href="' . G5_ADMIN_URL . '/cookie_list.php?sfl=mb_id&amp;stx=' . $mb_id . '" target="_blank" rel="nofollow">쿠키</a>';
			$menus['admin_member_point'] .= '<a href="' . G5_ADMIN_URL . '/candy_list.php?sfl=mb_id&amp;stx=' . $mb_id . '" target="_blank" rel="nofollow">캔디</a>';
			$menus['admin_member_point'] .= '<a href="' . G5_ADMIN_URL . '/roulette_list.php?sfl=mb_id&amp;stx=' . $mb_id . '" target="_blank" rel="nofollow">룰렛</a>';
			$menus['admin_member_point'] .= '<a href="' . G5_ADMIN_URL . '/box_list.php?sfl=mb_id&amp;stx=' . $mb_id . '" target="_blank" rel="nofollow">상자</a>';
			$menus['admin_member_point'] .= '<a href="' . G5_ADMIN_URL . '/time_list.php?sfl=mb_id&amp;stx=' . $mb_id . '" target="_blank" rel="nofollow">TIME</a>';
			$menus['admin_member_point'] .= '<a href="' . G5_ADMIN_URL . '/pass_list.php?sfl=mb_id&amp;stx=' . $mb_id . '" target="_blank" rel="nofollow">PASS</a>';
			$menus['admin_member_point'] .= '<a href="' . G5_ADMIN_URL . '/x2_list.php?sfl=mb_id&amp;stx=' . $mb_id . '" target="_blank" rel="nofollow">X2</a>';
        }
    }
...
function view_file_link($file, $width, $height, $content='')
{
    ...
    if (preg_match("/\.({$config['cf_image_extension']})$/i", $file) && isset($board['bo_table'])) {
        ...
        //$img = '<a href="'.$attr_href.'" target="_blank" class="view_image">'; CONUT
...
function member_delete($mb_id)
{
    ...
    $sql = " update {$g5['member_table']} set mb_password = '', mb_level = 1, mb_email = '', mb_homepage = '', mb_tel = '', mb_zip1 = '', mb_zip2 = '', mb_addr1 = '', mb_addr2 = '', mb_addr3 = '', mb_point = 0, mb_profile = '', mb_birth = '', mb_sex = '', mb_signature = '', mb_memo = '".date('Ymd', G5_SERVER_TIME)." 삭제함\n".sql_real_escape_string($mb['mb_memo'])."', mb_certify = '', mb_adult = 0, mb_dupinfo = '' where mb_id = '{$mb_id}' ";
